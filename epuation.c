...
Equation* getEquation(int i, int num) // get equation from user
{
	...
	eq-> count = num ;
	setCoefficients(eq, str ,num );
	return eq;

}

int getCount(char* str){ //get the number of * in the equation
	int count =0;
	char ch = '*';
...
void setCoefficients(Equation* eq, char* str,int num) //get Coefficients from equation
{
...
void getFreeNumber(char* cpstr, Equation* eq) { //get the free number from equation
